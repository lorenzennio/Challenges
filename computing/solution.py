...
fig, ax, p = plot_pixels((numer / denom).get())

fig.colorbar(
    p,
    ax=ax,
    shrink=0.8,
    label="fraction of sampled points in Mandelbrot set in each cell",
)

ax.axis([-0.15, -0.05, 0.8, 0.9])
fig.savefig("gpu_fraction.png")


fig, ax, p = plot_pixels(uncertainty.get())


fig.colorbar(p, ax=ax, shrink=0.8, label="uncertainty of Mandelbrot area in each cell")

ax.axis([-0.15, -0.05, 0.8, 0.9])

fig.savefig("gpu_uncertainty.png")


fig, ax, p = plot_pixels(denom.get())

fig.colorbar(p, ax=ax, shrink=0.8, label="number of points sampled each cell")

ax.axis([-0.15, -0.05, 0.8, 0.9])

fig.savefig("gpu_denom.png")
